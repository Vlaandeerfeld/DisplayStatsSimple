async function leagueFilters(){
	...
	await fetch('http://192.168.2.12:3000/fhmleague')
		.then(response => response.json())
		.then(data => {
			console.log(data)
			data.map(value => {
				return outputHTML += `<option value = ` + value.LeagueId + `>` + value.AbbrLeague + `</option>`;
			})
		});
...
async function teamFilters(league){
	let outputHTML = '';

	outputHTML += `<option value ='ALL'>ALL</option>`;

	await fetch('http://192.168.2.12:3000/fhmteams', {
		headers: {
			league: league
		}
	})
		...
		.then(data => {
			console.log(data)
			data.map(value => {
				return outputHTML += `<option value = ` + value.TeamId + `>` + value.Abbr + `</option>`;
			})
		});
...
async function seasonFilters(league){
	let outputHTML = '';
	...
	.then(data => {
		console.log(data)
		data.map(value => {
			return outputHTML += `<option value = ` + value.Season + `>` + value.Season + `</option>`;
		})
	});
...
async function teamTablesOverview(league, season, team){
	outputHTML = '';
	outputHTML += `<tbody><tr><th onclick = 'sortTable("teamTablesOverview", 0)'>City</th><th onclick = 'sortTable("teamTablesOverview", 1)'>Team</th><th onclick = 'sortTableNum("teamTablesOverview", 2)'>GP</th><th onclick = 'sortTableNum("teamTablesOverview", 3)'>Wins</th><th onclick = 'sortTableNum("teamTablesOverview", 4)'>Losses</th><th onclick = 'sortTableNum("teamTablesOverview", 5)'>OTL</th><th onclick = 'sortTableNum("teamTablesOverview", 6)'>Points</th></tr>`;
	
	console.log(team);
	await fetch('http://192.168.2.12:3000/fhmteam', {
		headers:{
			season: season,
			team: team,
			league: league,
	...
	.then(data => {
		data.map(value => {
			outputHTML += `<tr><td>${value.city}<td id = ${value.teamName} onclick = 'localStorage["currentLeague"] = document.getElementById("FHMleaguesstats").options[document.getElementById("FHMleaguesstats").options.selectedIndex].text; localStorage.setItem("currentTeam", document.getElementById("${value.teamName}").innerHTML); window.location.href = "FHMstatsteam.html";'>${value.teamName}</td><td>${value.GP}</td><td>${value.Wins}</td><td>${value.Losses}</td><td>${value.OTL}</td><td>${value.Points}</td></tr>`;
		})
	})
